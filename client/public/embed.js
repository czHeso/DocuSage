(() => {
  ...
  const initChatBot = () => {
    ...
    // the widget sits on a site in its own language, and everything else
    // visitor-facing here already is.
    const sourcesLabel =
      script.getAttribute("data-sources-label") || "Sources";
     ...
     *
     * Built with textContent throughout. A file name is chosen by whoever
     * uploaded the document and ends up in a page the operator does not
     * control, so it goes in as text and never as markup.
     */
    const appendSources = (messageElement, sources) => {
      if (!Array.isArray(sources) || sources.length === 0) return;

      const container = document.createElement("div");
      container.className = "docusage-sources";
    ...
    const sendMessage = async (message) => {
      ...
      try {
        ...
        // Add the bot's answer, with the documents it cited when the project
        // has citations switched on
