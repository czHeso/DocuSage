(() => {
  ...
  const initChatBot = () => {
    ...
    // prompt and the thank-you come from the project settings; these are the
    // field labels, which are the same for everyone and belong on the tag.
    ...
    const leadPromptFallback = "Leave us your email and we will get back to you.";
    const leadThankYouFallback = "Thank you, we will be in touch.";
    const leadErrorFallback = "The details could not be sent. Please try again.";
     ...
     *
     * Whether to show this is decided by the server, which sends leadCapture on
    ...
    const showLeadForm = (config, question) => {
      const container = document.createElement("div");
      container.className = "docusage-lead-form";
      ...
      prompt.textContent = config.prompt || leadPromptFallback;
      ...
      form.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(data.message || leadErrorFallback);
          }

          // The form is replaced rather than hidden, so there is no way to
          // submit it twice and no doubt about whether it went through.
          container.textContent = data.message || config.thankYou || leadThankYouFallback;
          container.style.background = "rgba(16,185,129,0.08)";
        } catch (submitError) {
          error.textContent = submitError.message || leadErrorFallback;
    ...
    const leadApiUrl = () => {
      const script =
        ...
        Array.from(document.querySelectorAll("script")).find(
          (candidate) => candidate.src && candidate.src.includes("embed.js"),
        );

      try {
        const origin = script && script.src ? new URL(script.src).origin : window.location.origin;
    ...
    const sendMessage = async (message) => {
      ...
      try {
        ...
        // wants to collect contact details at all.
        if (data.leadCapture) {
        ...
        console.error("DocuSage Chatbot Error details:", {
          ...
          // Add token information for easier diagnostics. This said embedToken,
          // which is declared nowhere - so every failed request threw a
          // ReferenceError out of its own error handler, and the diagnostics
          // that were supposed to explain the failure never printed.
